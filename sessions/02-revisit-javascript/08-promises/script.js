...
(function() {
  ...
  myProfile
    ...
    .then(data => {
      ...
      // 🎙️ Now return another promise which would resolve when both
      // 🎙️ are done, using Promise.all
      return Promise.all([repos, followers]);
    })
    .then(results => {
    ...
    // 🎙️ We can also destructure inline in function parameter
    .then(([repoData, followerData]) => {
      // 🎙️ Let's log them when both of them are done
      console.log(repoData);
      console.log(followerData);
    })
    // 🎙️ Finally a single catch handler for any possible errors
    .catch(e => {
      console.log('Error', e);
    });
})();
