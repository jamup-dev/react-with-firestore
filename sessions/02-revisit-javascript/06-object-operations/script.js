...
(function() {
  ...
  // 🎙️ If we change how we call the object, the scope is list
  try {
    const sayNameVar = profile.sayName;
    console.log(sayNameVar());
  } catch (e) {
    console.log(e);
  }
  // 🎙️ To `fix` this, we can hard-bind sayName to the object
  profile.sayName = profile.sayName.bind(profile);
  // 🎙️ Now if we do like before
  const sayNameVar = profile.sayName;
  console.log(sayNameVar());
  // 🎙️ Then it works. But we usually don't do things this way because
  // 🎙️  1. It is confusing.
  // 🎙️  2. It is not apparent what's happening.
  // 🎙️ We just call with proper scope.
})();
